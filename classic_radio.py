...
class Ui_f_dialog(object):
    ...
    def retranslateUi(self, f_dialog):
        ...
        linux_icon_path = "/usr/share/classic_radio/classic_radio.ico"
        if os.sys.platform == "linux" and os.path.exists(linux_icon_path):
            ico_path = linux_icon_path
        else:
            ico_path = "music.ico"
        f_dialog.setWindowIcon(QIcon(ico_path))
        # item.setSizeHint(QSize(500,0))
        # item.setMinimumSize(

    def qry_url(self):
        l_datetime = (
            self.f_dateEdit.dateTime().toPyDateTime().strftime("%Y%m%d")
        )  # 取得存檔時間
        l_datetime = str(l_datetime)
        html = urlopen(
            "http://www1.family977.com.tw/Reprogress_00.asp?strDay=%s#" % l_datetime
        )
        ...
        for item in soup1:
            for cell in item.find_all("tr"):
                ...
                print(l_list)
                l_i += 1
        self.f_table.resizeColumnsToContents()
    ...
    def createContextMenu(self, f_dialog):
        ...
        self.actionA = self.contextMenu.addAction("複製到剪貼簿")
        # 将动作与处理函数相关联
